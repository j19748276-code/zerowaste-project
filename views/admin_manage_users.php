...
<div class="container mx-auto">
    <h1 class="text-3xl font-bold mb-6">Gérer les Utilisateurs</h1>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody>
                    <?php if (empty($users)): ?>
                        <tr>
                            <td colspan="5" class="p-4 text-center text-gray-500">Aucun utilisateur n'a été trouvé.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($users as $user): ?>
                            <tr class="border-b hover:bg-gray-50">
                                <td class="p-4 font-medium"><?= htmlspecialchars($user['id']) ?></td>
                                <td class="p-4"><?= htmlspecialchars($user['username']) ?></td>
                                <td class="p-4"><?= htmlspecialchars($user['email']) ?></td>
                                <td class="p-4">
                                    <span class="px-3 py-1 text-xs font-medium rounded-full <?= $user['role'] === 'admin' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700' ?>">
                                ...
                                <td class="p-4 text-right">
                                    <div class="flex justify-end gap-3">
                                        <?php if ($user['id'] != $_SESSION['user_id']): ?>
                                            <a href="index.php?action=adminEditUser&id=<?= $user['id'] ?>" class="Btn Btn-edit">
                                                Edit
                                            ...
                                            <a href="#" onclick="openConfirmationModal('index.php?action=adminDeleteUser', '<?= $user['id'] ?>'); return false;" class="Btn Btn-delete">
                                                Delete
                                        ...
                                        <?php else: ?>
                                            <span class="text-gray-400 text-sm font-medium pr-4">Vous</span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
